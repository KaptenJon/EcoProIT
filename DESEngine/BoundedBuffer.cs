...
namespace React
{
    ...
    /// </summary>
    public class BoundedBuffer : Blocking<DesTask>, IBoundedBuffer
    {
        /// <summary>
        ...
        /// <summary>
        /// The wait queue for consumer desTasks.
        /// </summary>
        private IQueue<DesTask> _consumerQ;
        /// <summary>
        /// The wait queue for producere desTasks.
        /// </summary>
        private IQueue<DesTask> _producerQ;
        /// <summary>
        /// The number of items reserved by consumer desTasks.
        /// </summary>
        ...
        /// </summary>
        /// <remarks>
        /// When <see cref="Count"/> is zero (0), <see cref="DesTask"/>s
        /// returned by <see cref="Get"/> will block.  When
        /// <see cref="Count"/> is equal or greater than <see cref="Capacity"/>,
        /// <see cref="DesTask"/>s returned by <see cref="Put"/> will block.
        /// </remarks>
        /// <value>
        ...
        /// <see cref="DesTask"/> in order for an item to be removed from the
        ...
        /// </exception>
        /// <param name="consumer">
        /// The consumer <see cref="DesTask"/>.
        /// </param>
        /// <returns>
        /// A <see cref="DesTask"/> which will remove an item from the
        /// <see cref="BoundedBuffer"/> on behalf of
        /// <paramref name="consumer"/>.
        /// </returns>
        [BlockingMethod]
        public DesTask Get(DesTask consumer)
        {
            if (consumer == null)
        ...
        /// <see cref="DesTask"/> in order for <paramref name="item"/> to be
        ...
        /// </exception>
        /// <param name="producer">
        /// The producer <see cref="DesTask"/>.
        /// </param>
        /// <param name="item">
        /// The object to place into the <see cref="BoundedBuffer"/>.
        /// </param>
        /// <returns>
        /// A <see cref="DesTask"/> which will add an item to the
        /// <see cref="BoundedBuffer"/> on behalf of
        /// <paramref name="producer"/>.
        /// </returns>
        [BlockingMethod]
        public DesTask Put(DesTask producer, object item)
        {
            if (producer == null)
        ...
        public override DesTask[] GetBlockedTasks(int queueId)
        {
            ...
            {
                case AllQueues:
                    IQueue<DesTask>[] array = new IQueue<DesTask>[2];
                    array[0] = _consumerQ;
                    array[1] = _producerQ;
                    desTasks = GetBlockedTasks(array);
                    break;
                case ConsumerQueueId:
                    desTasks = GetBlockedTasks(_consumerQ);
                    break;
                case ProducerQueueId:
                    desTasks = GetBlockedTasks(_producerQ);
                    break;
                default:
                    throw new ArgumentException("Invalid queue id: " + queueId);
            }
        ...
        /// </returns>
        protected override IQueue<DesTask> CreateBlockingQueue(int queueId)
        {
            if (queueId == ConsumerQueueId)
                return new React.Queue.FifoQueue<DesTask>();
        ...
        /// <summary>
        /// Attempt to resume as many waiting consumer <see cref="DesTask"/>s as
        /// possible.
        /// </summary>
        ...
        /// <summary>
        /// Called by the <see cref="BufferGet"/> DesTask as well as the
        /// <see cref="ResumeWaitingConsumers"/> method to attempt to
        /// get an item from the <see cref="BoundedBuffer"/>.
        /// </summary>
        /// <param name="consumer">The <see cref="BufferGet"/> DesTask.</param>
        /// <returns>
        /// <b>true</b> if <paramref name="consumer"/> was blocked.
        /// </returns>
        internal bool GetObject(DesTask consumer)
        {

...
namespace EcoProIT.DataLayer
{
    ...
    public class GeneralConverter : IList<Double>, INotifyPropertyChanged, INotifyCollectionChanged
    {
        ...
        protected virtual void OnCollectionChanged(double changed, int index)
        {
        ...
        public static IEnumerable<Index> Indexes
        {
            get
            {
                ...
                var types = System.AppDomain.CurrentDomain.GetAssemblies()
                                                    ...
                                                    .Where(p => p.AssemblyQualifiedName != null && type.IsAssignableFrom(p) && p.IsClass && !p.IsAbstract).AsParallel();
               _indexes = types.Select(type1 => (Index) Activator.CreateInstance(Type.GetType(type1.AssemblyQualifiedName))).ToList();
                return _indexes;
            }
        }
    }

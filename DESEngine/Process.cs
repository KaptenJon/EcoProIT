...
namespace React
{
	...
	/// <see cref="DesTask"/>s that make up a process's sequence of steps.
	/// </summary>
	/// <remarks>
	...
	/// }
	/// 
	/// private IEnumerator<DesTask> StepsMethod(Process p, object data)
	/// {
	///     ulong delayTime = (ulong) data;
	...
	/// <see langword="null"/>.
	/// </param>
	public delegate IEnumerator<DesTask> ProcessSteps(Process process, object data);

	/// <summary>
	/// A <see cref="DesTask"/> implementation that uses an iterator method to
	/// support simulating complex or ulong-running processes.
	/// </summary>
	public class Process : DesTask
	{
		/// <summary>
		...
		/// </summary>
		private IEnumerator<DesTask> _steps;
		/// <summary>
		...
		/// <summary>
		/// Gets an <see cref="IEnumerator&lt;DesTask&gt;"/> that yields the processing
		/// steps.
		/// </summary>
		/// <remarks>
		/// This is a generator method that must be overridden by subclasses.  It
		/// must <c>yield</c> one or more <see cref="DesTask"/> instances which will
		/// perform each processing step.
		/// </remarks>
		/// <returns>
		/// An <see cref="IEnumerator&lt;T&gt;"/> (iterator) capable of
		/// yielding one or more <see cref="DesTask"/> instances that will
		/// perform actions on behalf of the <see cref="Process"/>.
		/// </returns>
		protected virtual IEnumerator<DesTask> GetProcessSteps()
		{
			if (_stepsfunc == null)
			{
		...
		/// <summary>
		/// Defer processing to allow another <see cref="DesTask"/> to run.
		/// </summary>
		/// <remarks>
		/// This method is used to temporarily suspend the current
		/// <see cref="Process"/> and allow another <see cref="DesTask"/> to
		...
		/// <see cref="DesTask"/> to re-activate it; a deferred
		...
		/// </returns>
		[BlockingMethod]
		public DesTask Suspend()
		{
			return this;
		}

		/*
		protected DesTask Acquire(IResource resource)
		{
		}

		protected DesTask Acquire(IResource [] resources)
		{
		}

		protected DesTask Acquire(IResource [] resources, bool acquireAll)
		{
		}

		protected DesTask Release(IResource resource)
		{
		}

		protected DesTask Release(IResource [] resources)
		{
		}

		protected DesTask WaitFor(DesTask blocker)
		{
		}

		protected DesTask WaitFor(DesTask [] blockers)
		{
		}
		*/
	}

...
namespace Sparrow.Chart
{
    ...
    public abstract class XAxis : AxisBase
    {
        ...
        internal void Update()
        {
            ...
            if (this.ActualHeight > 0 && this.ActualWidth > 0)
            {
                ...
                if (this.MLabels.Count == Labels.Count)
                {
                    ...
                    {
                        xAxisWidthPosition = this.DataToPoint(MStartValue + (MInterval/2 * k));
        ...
        override public string GetOriginalLabel(double value)
        {
            ...
            {
                case XType.Double:
                    if(StringFormat.ToLower() == "my special way")
                        if (value > 99.999)
                ...
                case XType.Category:
                    ...
                    if (Chart.ActualCategoryValues.Count > (int)value)
                        return Chart.ActualCategoryValues[(int)value];
                    else
                        return "";
                default:
                    return value.ToString(this.StringFormat);                    

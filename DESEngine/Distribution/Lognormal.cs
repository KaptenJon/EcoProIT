...
namespace React.Distribution
{
    ...
    public class Lognormal : NonUniform
    {
        ...
        public Lognormal(bool scaleandshape,double scale, double shape)
        {
            this.Mean = Math.Exp(scale+(Math.Pow(shape,2)/2));
            this.StandardDeviation = Math.Sqrt(Math.Exp(Math.Pow(shape, 2)) - 1)*Math.Exp(2*scale + Math.Pow(shape, 2));
        }
